...
TARGETS: dict[str, list[tuple[int, str, str]]] = {
    "00_NorthRia.txt": [(5094, "building_glassworks", "building_silk_plantation")],
    "01_SouthRia.txt": [(2035, "building_port", "building_silk_plantation")],
    "04_NorthAfrica.txt": [
...
def dedent_to(block: str, old_indent: str, new_indent: str) -> str:
    return ''.join(
        new_indent + line[len(old_indent):] if line.startswith(old_indent) else line
...
def move_known_nested(text: str, line_no: int, expected_outer: str, expected_inner: str) -> str:
    ...
    _, outer_close, _ = create_block(text, outer)
    ...
    if building_type(inner_raw) != expected_inner:
        raise RuntimeError(
            f"line {line_no}: expected inner {expected_inner}, got {building_type(inner_raw)}"
        )

    inner_raw = normalize_extracted_building(
        dedent_to(inner_raw, inner.group(1), outer.group(1))
    ).rstrip()

    # Remove the inserted block, then repair any token it had split.  Only after
    # this is it meaningful to validate the host building type (e.g. b/uilding).
    ...
    _, outer_close2, outer_raw2 = create_block(text, outer2)
    actual_outer = building_type(outer_raw2)
    if actual_outer != expected_outer:
        raise RuntimeError(
            f"line {line_no}: expected repaired outer {expected_outer}, got {actual_outer}"
        )

    return text[:outer_close2 + 1] + "\n\n" + inner_raw + text[outer_close2 + 1:]
...
def repair_special_structure(files: dict[str, str]) -> None:
    ...
    if files[p].count(old) != 2:
        raise RuntimeError(f"08_CentralIslands direct levels blocks: expected 2, found {files[p].count(old)}")
...
def main() -> None:
    ...
    # The source contains one split "building" token, so compare inventory only
    # after repairing a copy of all source text.
    repaired_before = {}
    for name, text in files.items():
        repaired_before[name], _ = repair_split_tokens(text)
    before_buildings = counts_by_building(repaired_before)
    ...
    print("remaining split-token repairs:", split_total)
